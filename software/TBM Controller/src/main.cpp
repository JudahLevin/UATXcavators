...
const int NUM_CHANNELS = sizeof(channels) / sizeof(Channel);
...
void updateChannel(int i, uint32_t now) {
  ...
  if (alarmCondition && !alarmLatched[i]) {
    if (violationStart[i] == 0) violationStart[i] = now;
  ...
  if (alarmLatched[i] && inRange(ch.clearLow) && currentValue[i] <= ch.clearHigh && currentValue[i] >= ch.clearLow) {
    ...
    if (safeStart[i] == 0) safeStart[i] = now;
...
void handleInterlocks() {
  ...
  for (int i = 0; i < NUM_CHANNELS; i++) {
    Channel &ch = channels[i];
    if (alarmLatched[i]) {
      if (String(ch.interlockAction).indexOf("Motor") >= 0) motorTrip = true;
      if (String(ch.interlockAction).indexOf("Actuator") >= 0) actuatorTrip = true;
    }
  }
  ...
  if (motorTrip || actuatorTrip) {
    Serial.println("\n⚡ INTERLOCK TRIGGERED ⚡");
    if (motorTrip) Serial.println("🔴 Motor Disabled");
    if (actuatorTrip) Serial.println("🔴 Actuator Disabled");
    Serial.println();
  }
}
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    cmd.trim();
    if (cmd.equalsIgnoreCase("reset")) manualReset();
  }
